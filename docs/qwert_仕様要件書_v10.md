...
- 画像表示（ローカルファイル参照、Tauriのassetプロトコル経由）
...
`'unsafe-inline'` style-src はMermaidが内部でインラインstyleを生成するため現実的に必要。ただし `unsafe-eval` は含めない。`asset:` プロトコルはTauriの protocol-asset feature によりvault配下の画像のみ許可する。

### Tauri capabilities最小化

| capability | 設定 | 根拠 |
|-----------|------|------|
| `fs` | vault配下のみ許可 | パストラバーサル防止、`canonicalize` + `starts_with` 検証を全ファイルアクセス関数の入口に配置 |
| `http` | デフォルト無効 | qwertは外部URL取得機能を持たない |
| `shell` | 全面不許可 | 任意コマンド実行経路を構造的に閉じる |
| `dialog` | ファイル選択のみ許可 | Vault選択時の OpenDialog のみ |
| `protocol-asset` | vault配下のみscope設定 | ローカル画像プレビュー用、画像バイナリ転送のJSONシリアライズ回避 |
...
# rmcp = { version = "0.16", features = ["server"] } # Phase 3: MCP
# tokio = { version = "1", features = ["full"] }     # Phase 3: MCP
```
...
- `.stignore` テンプレート提供（`.qwert/cache/` および `.qwert/pending-revision.json` を除外）
...
3. Android ファイルアクセス・権限処理（SAF対応）
4. Syncthing連携テスト（Nothing Phone 3a実機）
5. ソフトキーボード最適化
6. APK/AABパッケージング・配布
